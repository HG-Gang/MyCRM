		...
		if(_this.attr("target") == "_blank"){
			...
			if(that.hasTab(_this.find("cite").text()) == -1 && _this.siblings("dl.layui-nav-child").length == 0){
				...
				element.tabAdd(tabFilter, {
			        title : title,
			        content :"<iframe src='"+_this.attr("data-url")+"' data-id='"+tabIdIndex+"' name='layui-layer-iframe-" + _this.attr("data-key") +"'></frame>",
			        id : new Date().getTime(),
					name : "layui-layer-iframe-" + _this.attr("data-key")
			    })
				//当前窗口内容
				var curmenu = {
					...
					"layName" : "layui-layer-iframe-" + _this.attr("data-key")
				}
				menu.push(curmenu);
