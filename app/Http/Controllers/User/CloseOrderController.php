<?php
	...
	class CloseOrderController extends Abstract_Mt4service_Controller
	{
		
		public function close_order_browse ()
		{
			
			return view ('user.close_order.close_order_browse')->with (['_user_info' => $this->_user]);
		}
		
		public function closeOrderSearch (Request $request)
		{
			$result = array('rows' => '', 'total' => '');
			
			$_rs = $this->get_current_agents_close_order_id_list('page', $request);
			
			if (!empty($_rs)) {
				$result['rows'] = $_rs;
				$result['total'] = $this->get_current_agents_close_order_id_list ('count', $request);
				$_datasum = $this->get_current_agents_close_order_id_sum_data ($request);
				$result['footer'] = [[
					'ticket'        => '',
					'login'         => '',
					'symbol'        => '总计',
		...
		public function close_order_detail ($orderId, $orderType, $role)
		{
			$_rs = Mt4Trades::select('mt4_trades.*', 'user_trades.rec_comp_date', 'user_trades.voided', 'user_trades.comment')
				->leftJoin('user_trades', 'user_trades.ticket', ' = ',  'mt4_trades.TICKET')
				->where('mt4_trades.TICKET', $orderId)->where('mt4_trades.CLOSE_TIME', '>', '1970-01-01 00:00:00')
				->where('mt4_trades.CONV_RATE1', '<>', 0)->whereIn('mt4_trades.CMD', array (0,1,2,3,4,5))->get()->toArray();
			
			return view ('user.close_order.close_order_detail')->with (['_rs' => $_rs, '_orderType' => $orderType, '_role' => $role]);
		}
		
		protected function get_current_agents_close_order_id_list ($search, $request)
		{
			...
			")->whereIn('MT4_TRADES.LOGIN',function ($query) use($loginId) {
				...
				->where(function ($subWhere) use ($userId, $orderId, $is_coercion, $startdate, $enddate) {
					if (!empty($startdate) && !empty($enddate) && $this->_exte_is_Date ($startdate) && $this->_exte_is_Date ($enddate)) {
						$subWhere->whereBetween('mt4_trades.CLOSE_TIME', [$startdate .' 00:00:00', $enddate . ' 23:59:59']);
					} else {
						if(!empty($startdate) && $this->_exte_is_Date ($startdate)) {
							$subWhere->where('mt4_trades.CLOSE_TIME',  '>= ', $startdate .' 23:59:59');
						}
						if(!empty($enddate) && $this->_exte_is_Date ($enddate)) {
							$subWhere->where('mt4_trades.CLOSE_TIME', '<', $enddate .' 00:00:00');
						}
					}
					...
					if ($is_coercion != '' && $is_coercion == 'Yes') {
						$subWhere->where('mt4_trades.COMMENT', 'like', 'so%');
					}
				});
			
			if ($search == 'page') {
				$id_list = $query_sql->skip($this->_offset)->take($this->_pageSize)->orderBy('CLOSE_TIME', 'desc')->get()->toArray();
		...
		protected function get_current_agents_close_order_id_sum_data ($request)
		{
