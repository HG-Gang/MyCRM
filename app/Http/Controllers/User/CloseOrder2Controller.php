<?php
	...
	class CloseOrder2Controller extends Abstract_Mt4service_Controller
	{
		public function close_order2_browse ()
		{
			return view ('user.close_order2.close_order2_browse')->with (['_user_info' => $this->_user]);
		}
		
		public function closeOrder2Search (Request $request)
		{
			$result = array('rows' => '', 'total' => '');
			
			$_rs = $this->get_self_loginId_close_order_id_list('page', $request);
			
			if (!empty($_rs)) {
				$result['rows'] = $_rs;
				$result['total'] = $this->get_self_loginId_close_order_id_list ('count', $request);
				$_datasum = $this->get_self_loginId_close_order_id_sum_data ($request);
				$result['footer'] = [[
					'ticket'        => '',
					'login'         => '',
					'symbol'        => '总计',
		...
		protected function get_self_loginId_close_order_id_list ($search, $request)
		{
			...
			->where(function ($subWhere) use ($orderId, $is_coercion, $startdate, $enddate) {
		...
		protected function get_self_loginId_close_order_id_sum_data($request)
		{
			$orderId        = $request->orderId;
			$is_coercion    = $request->is_coercion;
			$startdate      = $request->startdate;
