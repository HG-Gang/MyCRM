...
@section('content')
	...
	<form class="layui-form" action="" id="AdminCloseOrderForm" style="margin-top: 8px;">
	...
	<script type="text/javascript">
		function dataGridConfig() {
			...
			config.DataColumns = [[
				{field:'ticket' ,title:'{{ trans ('systemlanguage.close_order_ticket_id') }}', width:100, align:'center', formatter: function (value, rowData, rowIndex) {
					if (value) {
						return "<font  onclick='orderIdDetail("+ value +", "+'"closeOrder"'+", "+'"admin"'+")' style='color: blue; cursor: pointer;' title='查看订单详情'>" + value + "</font>";
					}
				}},
				{field:'login' ,title:'{{ trans ('systemlanguage.close_order_user_id') }}', width:100, align:'center', formatter: function (value, rowData, rowIndex) {
					if ('{{ $_permit }}' == 4) {
						...
						return "<font  onclick='userIdDetail("+ value +", "+'"admin"'+")'style='color: blue; cursor: pointer;' title='查看账号详情'>" + value + "</font>";
					}
				}},
				{field:'symbol' ,title:'{{ trans ('systemlanguage.close_order_symbol_type') }}', width:100, align:'center',},
				{field:'cmd' ,title:'{{ trans ('systemlanguage.close_order_cmd_type') }}', width:100, formatter: function (value, rowData, rowIndex) {
					if (rowData.ticket != '') {
		...
		function createTable() {
			...
			pagerData = new $.WidgetPage({
				reqUrl: route_prefix() + '/order/closeListSearch',
				tableId: 'data_list',
				formId: 'AdminCloseOrderForm',
				method: 'post',
