    ...
    <script>
        function dataGridConfig() {
            ...
            config.DataColumns = [[
                ...
                {field:'actapplyamount' ,title:'{{ trans ('systemlanguage.account_apply_actapplyamount_RMB') }}', width:100, align:'center', formatter: function (value, rowData, rowIndex) {
                    return parseFloatToFixed(value);
                }},
                {field:'actdraw' ,title:'{{ trans ('systemlanguage.account_apply_actdraw_RMB') }}', width:100, align:'center', formatter: function (value, rowData, rowIndex) {
                    return parseFloatToFixed(value);
                }},
                {field:'drawrate' ,title:'{{ trans ('systemlanguage.account_apply_drawrate') }}', width:100, align:'center',},
                {field:'drawpoundage' ,title:'{{ trans ('systemlanguage.account_apply_drawpoundage_RMB') }}', width:100, align:'center', formatter: function (value, rowData, rowIndex) {
                    return parseFloatToFixed(value);
                }},
                {field:'orderIdOTC' ,title:'{{ trans ('systemlanguage.account_apply_order_OTC') }}', width:180, align:'center',},
                ...
                {field:'rec_crt_date' ,title:'{{ trans ('systemlanguage.account_rec_crt_date') }}', width:100, align:'center',},
                    ...
                    if (rowData.applystatus == "0") {
                        var html = "";
                        if (rowData.orderIdOTC != "") {
                            html += '<a href="javascript:;" onclick="withdrawOrderIdDetail('+ rowData.record_id +', '+ rowData.mt4_ticket +')" class="l-btn l-btn-small l-btn-plain" style="color: blue;">' +
                                        '<span class="l-btn-left l-btn-icon-left">' +
                                            '<span class="l-btn-text">操作</span>' +
                                            '<span class="l-btn-icon icon-edit">&nbsp;</span>' +
                                        '</span>'+
                                    '</a>'
                        } /*else if (rowData.orderIdOTCstatus == '2') {
                            html += '<a href="javascript:;" onclick="OTCwithdrawOrderIdDetail('+ rowData.record_id +', '+ rowData.userId +', '+"'/amount/OTCwithdrawOrderIdDetail'"+', '+ "'{{ csrf_token() }}'" +')" class="l-btn l-btn-small l-btn-plain" style="color: blue;">' +
                                        '<span class="l-btn-left l-btn-icon-left">' +
                                            '<span class="l-btn-text">查看订单</span>' +
                            ...
                            html += '<a href="javascript:;" onclick="withdrawOrderIdDetail('+ rowData.record_id +', '+ rowData.mt4_ticket +')" class="l-btn l-btn-small l-btn-plain" style="color: blue;">' +
                                        ...
                                        '</span>'+
                                    '</a>'+
                                    '<a href="javascript:;" onclick="GenerateOTCorder('+ rowData.record_id +', '+ rowData.userId +', '+"'/amount/generate_OTCorder'"+', '+ "'{{ csrf_token() }}'" +')" class="l-btn l-btn-small l-btn-plain" style="color: blue;">' +
                                        '<span class="l-btn-left l-btn-icon-left">' +
                                            '<span class="l-btn-text">生成订单</span>' +
