...
@section('content')
	<form class="layui-form" action="" id="realtimeRebateForm" style="margin-top: 8px;">
	...
	<script type="text/javascript">
		function dataGridConfig() {
			...
			config.DataColumns = [[
				...
				{field:'login' ,title:'{{ trans ('systemlanguage.real_rebate_user_id') }}', width:100, align: 'center', formatter: function (value, rowData, rowIndex) {
					if (value && value == '{{ $_user_info['user_id'] }}') {
						return "<span style='color: #1E90FF'>" + value + "</span>";
					}
		...
		function DbClickEditAccountInfo(rowIndex, rowData) {
			...
			show_rebate_order_detail(orderNo, 'agents');
		}
		
		function createTable() {
			var config = dataGridConfig(), pagerData;
			pagerData = new $.WidgetPage({
				reqUrl: '/user/realtime/realtimeRebateSearch',
				tableId: 'data_list',
				formId: 'realtimeRebateForm',
				method: 'post',
